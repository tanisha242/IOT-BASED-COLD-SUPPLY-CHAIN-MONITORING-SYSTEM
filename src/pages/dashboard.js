...
export default function Dashboard() {
  ...
  const [alertCount, setAlertCount] = useState(
    parseInt(localStorage.getItem("alertCount") || "0")
  );

  const lastAlertTime = useRef(
    parseInt(localStorage.getItem("lastAlertTime") || "0")
  );
  ...
  useEffect(() => {
    ...
    function generateDemoData() {
      ...
      BOXES.forEach(box => {
        ...
        const forceAlert =
          box.deviceId === "BOX-DEL-02" ||
          box.deviceId === "BOX-MUM-03";

        const temp =
          6 + (seed % 10) + (Math.random() * 2 - 1);

        const hum =
          40 + (seed % 30) + (Math.random() * 3 - 1);
    ...
    <div className="dashboard-page">
      ...
      <section className="summary-row">
        ...
        <div className="summary-card">
          ...
          <div className="big">{BOXES.length}</div>
        </div>

        <div className="summary-card">
          <div className="label">Active</div>
          <div className="big">
            {latest["BOX-LKO-01"] ? 1 : 0}
          </div>
        </div>

        <div className="summary-card">
          <div className="label">Alerts</div>
          <div className="big">{alertCount}</div>
        </div>
